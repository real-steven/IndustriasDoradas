alter table app.sync_changes
  drop constraint sync_changes_entity_type_check;

alter table app.sync_changes
  add constraint sync_changes_entity_type_check check (
    entity_type in (
      'SUPPLIER', 'WORKER', 'PRODUCTION_LINE', 'LINE_COMPONENT',
      'STATION', 'STATION_LINE_SCOPE', 'SHIPMENT',
      'RESPONSIBILITY_ASSIGNMENT', 'ADMINISTRATIVE_CORRECTION'
    )
  );

create function app.capture_administrative_correction_v1()
returns trigger
language plpgsql
security definer
...
begin
  if new.organization_id is null
     ...
     or new.action <> 'business.mutation'
...
revoke all on function app.capture_administrative_correction_v1()
  from public, anon, authenticated;
grant execute on function app.capture_administrative_correction_v1()
  to service_role;

create trigger trg_audit_events_administrative_correction
after insert on app.audit_events
for each row execute function app.capture_administrative_correction_v1();
